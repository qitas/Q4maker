...
class Planner {
  ...
  private:
    ...
    /**
     * The current position of the tool in absolute steps / Recalculated if any axis_steps_per_mm are changed by gcode
     */
  ...
  private:
    ...
    /**
     * Calculate the distance (not time) it takes to accelerate from initial_rate to target_rate using the given acceleration:
     */
    ...
    /**
     * Return the point at which you must start braking (at the rate of -'acceleration') if you start at 'initial_rate', accelerate (until reaching the point), and want to end at
