...
const char errormagic[] PROGMEM = "Painter Error:"; 
const char echomagic[] PROGMEM = "Painter echo:";
...
#include "bed_vector_3.h"
#if ENABLED(AUTO_BED_LEVELING_GRID)
#include "bed_qr_solve.h"
#endif
...
bool enqueue_and_echo_command(const char* cmd, bool say_ok /*=false*/ ) {
  ...
  void setup_filrunoutpin() {
		...
		pinMode(FIL_1_RUNOUT_PIN, INPUT);
		pinMode(FIL_2_RUNOUT_PIN, INPUT);
		pinMode(FIL_3_RUNOUT_PIN, INPUT);
		#if ENABLED(ENDSTOPPULLUP_FIL_RUNOUT)
		WRITE(FIL_RUNOUT_PIN, HIGH);
		WRITE(FIL_1_RUNOUT_PIN, HIGH);
		WRITE(FIL_2_RUNOUT_PIN, HIGH);
		WRITE(FIL_3_RUNOUT_PIN, HIGH);
		#endif
  }
#endif
...
void setup() {
  ...
  SERIAL_PROTOCOLLNPGM("Now Painter running(setup)");
  SERIAL_ECHO_START;
  ...
  SERIAL_ECHOLNPGM(" Painter " SHORT_BUILD_VERSION);
  ...
  setup_photpin();   // OUT_WRITE(PHOTOGRAPH_PIN, LOW);
  servo_init();
  ...
  #if ENABLED(SHOW_LED)
    #if ENABLED(WS8212A_PIN)
	  pinMode(WS8212A_PIN, OUTPUT);
	  digitalWrite(WS8212A_PIN, LOW);
    #endif
    #if ENABLED(WS8212B_PIN)
	  pinMode(WS8212B_PIN, OUTPUT);
	  digitalWrite(WS8212B_PIN, LOW);
    #endif
  #endif
  
       // analogWrite(HEATER_BED_PIN,255);
      // delay(1000);
     //  analogWrite(HEATER_BED_PIN,55);
...
inline void get_serial_commands() {
    ...
    static boolean serial_comment_mode = false;
                 // If the command buffer is empty for too long, send "wait" to indicate Marlin is still waiting.
...
void gcode_get_destination() {
  ...
  #if ENABLED(PRINTCOUNTER)  //淇濈暀鐩稿叧鏁版嵁
    if (!DEBUGGING(DRYRUN))
...
void manage_inactivity(bool ignore_stepper_queue/*=false*/) {
	  ...
	  if ((IS_SD_PRINTING || print_job_timer.isRunning()) && !(READ(FIL_1_RUNOUT_PIN) ^ FIL_RUNOUT_INVERTING))
		handle_filament_runout();
	  if ((IS_SD_PRINTING || print_job_timer.isRunning()) && !(READ(FIL_2_RUNOUT_PIN) ^ FIL_RUNOUT_INVERTING))
		handle_filament_runout();
	  if ((IS_SD_PRINTING || print_job_timer.isRunning()) && !(READ(FIL_3_RUNOUT_PIN) ^ FIL_RUNOUT_INVERTING))
		handle_filament_runout();
  #endif
